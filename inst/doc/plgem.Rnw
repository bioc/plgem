...
A \emph{wrapper} function (called \Rfunction{run.plgem}) is provided in the
package, which performs all the necessary steps to obtain a list of
differentially expressed genes or proteins (DEG), starting from a dataset of
class \Rclass{ExpressionSet}.
...
The first step is to \emph{fit the model} to the microarray or proteomics
...
condition C (i.e. \Rfunarg{fit.condition = 1}), because it contains 4 replicates.
...
Finally, setting \Rfunarg{plot.file = TRUE} saves a png image to a file instead of
...
The next step is the \emph{computation of the signal-to-noise ratio} (STN)
statistics for the detection of differential expression. The STN is determined
...
the \Rfunarg{covariateNumb} covariate defined in the \Robject{phenoData} slot of
the \Robject{ExpressionSet}) are compared to the first condition (baseline). If
the condition to be treated as the baseline is not the first one, you can change
this by modifying the argument \Rfunarg{baseline.condition}. A matrix of
...
null-hypothesis of not differential expression, a \emph{resampled statistic}
...
\Rfunarg{automatic}, the number of iterations is automatically determined based
...
Next, \emph{p-values are calculated} for each observed STN value via a call to
function \Rfunction{plgem.pValue}. Resampled STN values are also required by this
...
\subsection{Detection of differentially expressed genes or proteins}

Finally, \emph{DEG are selected} at the given significance level
\Rfunarg{delta} via a call to function \Rfunction{plgem.deg}. The chosen value of
\Rfunarg{delta} can be seen as an estimate of the False Positive Rate (FPR).
Therefore, in case of a microarray dataset with 10000 genes of which not a
single one is truly differentially expressed, choosing a \Rfunction{delta}
...
of different significance levels \Rfunction{delta} used as input. In this case the
...
In each list-item the values are the observed STN and the names are the IDs of the
